...
def run_handler(args: argparse.Namespace, stdout: IO, stderr: IO) -> None:
    ...
    try:
        with trace(args.config):
            ...
            if args.m:
                globals_collector[args.script_path] = runpy.run_module(args.script_path, run_name='__main__', alter_sys=True)
            else:
                globals_collector[args.script_path] = runpy.run_path(args.script_path, run_name='__main__')
    finally:
        sys.argv = old_argv
...
class MonkeytypeAssistant:
    ...
    def get_relative_path_to_py_file_or_package():
        ...
        init_dir = Path(__file__).resolve().parent.as_posix()
        if len(sys.argv) > 1:
            file_ran_on = sys.argv[1]
            try:
                exists = Path(file_ran_on).exists()
            except Exception as e:
                exists = False
            if exists:
                init_dir = Path(file_ran_on).resolve().parent().as_posix()
        filepath = get_filepath(prompt=prompt, init_dir=init_dir)
        return filepath
...
if __name__ == "__main__":
    show_info_message(title="Starting script",message="Now going to start create_monkeytype_patch.py's __name__=='__main__' method")
